...
void update_posview(player_pos_view *posview, char*** blocks, SDL *sdl) {
#endif
	float move_eps = 0.15;
	float tilt_epsX = 0.03;
	float tilt_epsY = 0.02;
	...
	if (key_is_pressed(XK_Up)) {
		posview->view.psi += tilt_epsY;
	}
	if (key_is_pressed(XK_Down)) {
		posview->view.psi -= tilt_epsY;
	}
	if (key_is_pressed(XK_Left)) {
		posview->view.phi -= tilt_epsX;
	}
...
int main(void) {
	...
	while (i >= 0) {
		...
		if (have_current_block) {
			current_block_c = blocks[current_block_z][current_block_y][current_block_x];
			blocks[current_block_z][current_block_y][current_block_x] = 'o';
			if (place_block_allowed && sdl->keys[SDL_SCANCODE_SPACE]) {
			...
			else if (!sdl->keys[SDL_SCANCODE_SPACE]) {
				place_block_allowed = 1;
			}
		}
		else if (!sdl->keys[SDL_SCANCODE_SPACE]){
